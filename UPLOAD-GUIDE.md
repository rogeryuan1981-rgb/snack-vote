...
`supabase/upgrade-20260825-live-unlocked-purchase-plan.sql`

採購建議建立後，只要尚未鎖定，提名、取消提名、一般投票或商品啟用狀態異動，都會自動重算入選品項、名次、票數及建議數量。採購排名只統計前台結果中目前仍有效、仍有提名的商品，因此不會再讓已停用、已刪除或已取消全部提名的舊票占用名次。持續入選商品會保留人工採購單價，新進榜商品才使用參考價；鎖定後則不再受異動影響。既有採購草稿會由 SQL 自動納入同步；從未產生過清單的活動仍需由管理者先按一次「產生採購建議」。
