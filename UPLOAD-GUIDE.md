...
- 採購工作區可同時保留多個活動與地點的清單，並優先顯示尚未完成的舊活動。
- 商品按讚／倒讚為跨期長期偏好；每位同仁每項商品只能擇一，且可取消或切換。
- 同仁、活動與商品皆可設定複數地點；既有單一地點資料會自動移轉，同仁只會進入地點相符的活動並看到適用商品。
- 同一位同仁若同時符合多個進行中活動，可在前台切換活動，不會把不同辦公地點的內容混在一起。
- 意見問卷已移除重複的「建議類別」，流程改為三項滿意度評分後直接填寫修改建議。
