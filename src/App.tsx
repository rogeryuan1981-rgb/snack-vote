...
import { PurchasePdfDownload } from "./PurchasePdfDownload";
import { purchaseTotals, validPurchaseDiscount } from "./lib/purchase-totals";
type WorkLocation = {
    id: string;
    name: string;
    active: boolean;
...
type Campaign = {
    ...
    purchase_at: string;
    status: string;
    purchase_discount_total?: number;
...
function imageTransformStyle(x: number, y: number, zoom: number) { const panX = (Number(x ?? 50) - 50) / 2; const panY = (Number(y ?? 50) - 50) / 2; return { objectPosition: "50% 50%", transform: `translate(${panX}%, ${panY}%) scale(${Number(zoom ?? 100) / 100})`, transformOrigin: "50% 50%" }; }
function productImageStyle(product: Pick<Product, "image_position_x" | "image_position_y" | "image_fit" | "image_zoom">) { return { ...imageTransformStyle(product.image_position_x, product.image_position_y, product.image_zoom), objectFit: product.image_fit === "cover" ? "cover" as const : "contain" as const }; }
function ImageFramingControls({ preview, x, y, fit, zoom, onChange }: { preview: string; x: number; y: number; fit: "cover" | "contain"; zoom: number; onChange: (next: { x?: number; y?: number; fit?: "cover" | "contain"; zoom?: number }) => void; }) {
    if (!preview) return null;
    return <div className="image-framing-panel"><div className="product-visual image-framing-preview has-image"><img src={preview} alt="商品圖片顯示預覽" style={{ ...imageTransformStyle(x, y, zoom), objectFit: fit }}/><span>前台商品卡片實際畫面</span></div><div className="image-framing-controls"><div className="fit-choice"><button type="button" className={fit === "cover" ? "active" : ""} onClick={() => onChange({ fit: "cover", zoom: 100, x: 50, y: 50 })}>填滿視窗</button><button type="button" className={fit === "contain" ? "active" : ""} onClick={() => onChange({ fit: "contain", zoom: 100, x: 50, y: 50 })}>完整顯示</button></div><label><span>圖片縮放</span><input type="range" min="50" max="200" step="5" value={zoom} onChange={event => onChange({ zoom: Number(event.target.value) })}/><output>{zoom}%</output></label><label><span>左右位置</span><input type="range" min="0" max="100" value={x} onChange={event => onChange({ x: Number(event.target.value) })}/><output>{x}%</output></label><label><span>上下位置</span><input type="range" min="0" max="100" value={y} onChange={event => onChange({ y: Number(event.target.value) })}/><output>{y}%</output></label><small>縮放範圍為 50%～200%，預設 100%；左右與上下會直接移動圖片，50% 為置中。</small></div></div>;
}
function ProductImageUploadField({ compact = false }: { compact?: boolean }) {
    ...
    useEffect(() => { const form = root.current?.closest("form"); const reset = () => { setPreview(current => { if (current.startsWith("blob:")) URL.revokeObjectURL(current); return ""; }); setX(50); setY(50); setFit("contain"); setZoom(100); }; form?.addEventListener("reset", reset); return () => form?.removeEventListener("reset", reset); }, []);
    ...
    return <div ref={root} className={`image-upload-framing ${compact ? "compact" : ""}`}><label className={compact ? "compact-file" : "image-input"}>{preview ? "更換商品圖" : "選擇商品圖"}<input name="image" type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={event => choose(event.target.files?.[0] ?? null)}/></label><input type="hidden" name="image_position_x" value={x}/><input type="hidden" name="image_position_y" value={y}/><input type="hidden" name="image_fit" value={fit}/><input type="hidden" name="image_zoom" value={zoom}/><ImageFramingControls preview={preview} x={x} y={y} fit={fit} zoom={zoom} onChange={next => { if (next.x != null) setX(next.x); if (next.y != null) setY(next.y); if (next.fit) setFit(next.fit); if (next.zoom != null) setZoom(next.zoom); }}/></div>;
}
...
function Login() {
    ...
    async function sendOtp(e?: FormEvent, confirmNewDevice = true) {
        ...
        setBusy(false);
        if (error)
            return showError(error.message.includes("rate limit") ? "登入信寄送次數過於頻繁，請稍後再試。" : error.message);
        setEmail(normalized);
        setToken("");
        setStep("otp");
        setResendIn(60);
        setMessage("登入信已寄出，請查看信箱。");
    }
    ...
    <section className="auth-panel">
      ...
      <div className="auth-card">
        ...
        {step === "email" && <form onSubmit={sendOtp}>
          <h2>使用公司 Email 登入</h2>
          <p>不需要密碼。輸入員工名單中的 Email，我們會寄送安全登入信；首次登入可直接點擊信中連結，之後則輸入驗證碼。</p>
          ...
          {deviceControl}
          <button className="auth-submit" disabled={busy}>{busy ? "寄送中…" : "寄送登入信"}</button>
        </form>}
...
function EmployeeApp({ employee, route }: {
    ...
    async function savePurchaseReview(campaignId: string, productId: string, body: string) { const clean = body.trim(); if (!clean)
        return notify("請輸入心得內容"); const { error } = await supabase.from("purchase_reviews").upsert({ campaign_id: campaignId, product_id: productId, employee_id: employee.id, author_name: employee.name, body: clean }, { onConflict: "campaign_id,product_id,employee_id" }); if (error)
        return notify(error.message); notify("心得已儲存，其他同仁現在可以看到了"); await load(true); }
    async function deletePurchaseReview(review: PurchaseReview) { if (!await askConfirm({ title: "刪除這則心得？", items: ["刪除後其他同仁將無法再看到這則心得。"], confirmLabel: "確認刪除", danger: true }))
    ...
    async function addCustomProduct(e: FormEvent<HTMLFormElement>) { e.preventDefault(); if (!campaign || phaseOf(campaign) !== "nomination")
        return notify("活動尚未進入提名階段，目前只能瀏覽商品"); if (draftNominations.length >= campaign.nomination_limit)
        return notify(`已達提名上限 ${campaign!.nomination_limit} 項`); const form = new FormData(e.currentTarget); const size = String(form.get("size") || "").trim(); if (/^\d+(?:\.\d+)?$/.test(size))
        return notify("規格不能只填數字；請加上單位，例如 45g、24入。若這是價錢，請填在綠色的 NT$ 參考價格欄位。"); const file = form.get("image") instanceof File && form.get("image") as File; try {
        ...
        if (file && file.size > 0) {
            ...
            const framed = await supabase.rpc("set_product_image_framing", { p_product_id: data, p_position_x: Number(form.get("image_position_x") || 50), p_position_y: Number(form.get("image_position_y") || 50), p_fit: String(form.get("image_fit") || "contain"), p_zoom: Number(form.get("image_zoom") || 100) });
            if (framed.error)
                throw framed.error;
        }
    ...
    const copy = phase === "nomination" ? ["商品提名中", "把想吃的，放進本月候選清單", `截止前可隨時更換；每項提名固定使用 1 票。`] : phase === "voting" ? ["具名投票中", "本月零食，現在由大家決定", `你共有 ${campaign.vote_limit} 票；提名票固定保留，其餘票可在截止前調整。`] : phase === "results" ? ["結果揭曉中", "票數已鎖定，看看本月結果", "最終排名、票數與投票同仁皆已公開。"] : phase === "upcoming" ? ["本期尚未開始", "本月零食活動即將開始", `提名將於 ${shortDate(campaign.start_at)} 開放；你可以先瀏覽本期商品。`] : ["安排採購中", "本月採購清單整理中", "管理者正依排名、預算與實際售價確認採購數量。"];
    const catalogHeading = phase === "nomination" ? "今天想提名哪一款？" : phase === "upcoming" ? "先看看本期有哪些商品" : phase === "voting" ? "看看大家正在支持誰" : "本期最終票選結果";
    return <ShellHeader employee={employee}>{availableCampaigns.length > 1 && <div className="employee-campaign-switch"><span>目前活動</span><select value={campaign.id} onChange={event => { setSelectedEmployeeCampaignId(event.target.value); setCustomOpen(false); }}>{availableCampaigns.map(row => <option key={row.id} value={row.id}>{row.label}</option>)}</select><small>你同時符合多個活動，可在此切換。</small></div>}<section className="hero"><div className="hero-main"><div className="eyebrow"><span className="live-dot"/>{copy[0]} · {campaign.label}</div><h1>{copy[1]}</h1><p>{copy[2]}</p>{campaign.description?.trim() && <div className="campaign-brief"><span>本期活動說明</span><p>{campaign.description}</p></div>}<div className="quota-row"><div className="quota-card"><span>我的提名</span><strong>{draftNominations.length}<small>／{campaign.nomination_limit}</small></strong></div><div className="quota-card"><span>我的票數</span><strong>{myVotes.length}<small>／{campaign.vote_limit}</small></strong></div></div></div><Timeline campaign={campaign} phase={phase}/></section>{campaign.purchase_plan_locked_at && <EmployeePurchasePlan campaign={campaign} products={products} purchases={purchases}/>}<section className="content-head"><div><p className="section-kicker">SNACK CATALOG</p><h2>{catalogHeading}</h2></div>{phase === "nomination" && <button className="custom-product-button" onClick={() => setCustomOpen(x => !x)}>＋ 找不到商品？自行新增</button>}</section>{phase === "nomination" && customOpen && <form className="custom-product-form" onSubmit={addCustomProduct}><div className="custom-form-intro"><strong>新增商品並提名</strong><small>請依欄位標題填寫；送出後會先列為待審商品，管理者確認後才會正式加入。</small></div><label className="custom-field custom-field-name"><span>商品名稱 <b>必填</b></span><input required name="name" placeholder="例如：健達繽紛樂巧克力"/></label><label className="custom-field"><span>品牌 <small>選填</small></span><input name="brand" placeholder="例如：Kinder"/></label><label className="custom-field"><span>商品分類 <b>必填</b></span><select required name="category" defaultValue=""><option value="" disabled>請選擇分類</option>{categories.slice(1).map(x => <option key={x}>{x}</option>)}</select></label><label className="custom-field custom-size-field"><span>規格 <small>選填，不是價格</small></span><input name="size" placeholder="例如：100g、24入" maxLength={80}/></label><label className="custom-field custom-price-field"><span>參考價格 <small>選填</small></span><div className="custom-price-input"><i>NT$</i><input name="reference_price" type="number" min="0" step="1" inputMode="numeric" placeholder="例如：45"/></div></label><ProductImageUploadField/><button className="custom-submit">確認資料並送出提名</button></form>}<div className="filters filters-with-sort"><label className="search"><span>⌕</span><input value={query} onChange={e => setQuery(e.target.value)} placeholder="搜尋商品、品牌或分類"/></label><div className="category-list">{categories.map(c => <button key={c} className={c === category ? "active" : ""} onClick={() => setCategory(c)}>{c}</button>)}</div>{finalRanking ? <div className="final-ranking-control"><span>顯示順序</span><strong>依最終名次</strong><small>同票並列名次</small></div> : <label className="sort-control"><span>排序方式</span><select value={sortMode} onChange={e => setSortMode(e.target.value as ProductSort)}><option value="smart">智慧排序</option><option value="votes">票數高到低</option><option value="name">商品名稱</option><option value="price-asc">價格低到高</option><option value="price-desc">價格高到低</option></select><small>我的提名／投票優先</small></label>}</div><section className="product-grid">{shown.map((p, index) => { const ns = nominations.filter(n => n.product_id === p.id); const vs = votes.filter(v => v.product_id === p.id); const pcs = comments.filter(c => c.product_id === p.id); const selected = draftNominations.includes(p.id); const voted = myVotes.some(v => v.product_id === p.id); return <article className="product-card" key={p.id}><div className={`product-visual ${tones[index % tones.length]} ${p.image_path ? "has-image" : ""}`}>{p.image_path ? <img src={productImageUrl(p.image_path)} alt={`${p.brand} ${p.name}`} style={productImageStyle(p)}/> : <span>{icons[p.category] ?? "✦"}</span>}<small>{p.category}</small>{rankMap.has(p.id) && ["voting", "results", "purchase"].includes(phase) && <b>第 {rankMap.get(p.id)} 名</b>}</div><div className="product-body"><div className="brand-line"><span>{p.brand}{p.approval_status === "pending" ? " · 待管理者審核" : ""}</span><strong>{p.reference_price == null ? "待確認" : `參考 NT$ ${Number(p.reference_price).toLocaleString()}`}</strong></div><h3>{p.name}</h3><p>{p.size}</p><ProductReactionControls productId={p.id} employeeId={employee.id} reactions={productReactions} onToggle={toggleProductReaction}/>{ns.length > 0 && <div className="nominator"><span>{ns.map(n => n.nominator_name).join("、")} 提名</span></div>}{["voting", "results", "purchase"].includes(phase) && <div className="voter-line"><strong>第 {rankMap.get(p.id)} 名 · {vs.length} 票</strong><span>{vs.map(v => v.voter_name).join("、") || "尚無投票"}</span></div>}<div className="card-actions">{phase === "nomination" ? <button className={`primary-action ${selected ? "selected" : ""}`} onClick={() => toggleNomination(p)}>{selected ? "✓ 已提名（可取消）" : "＋ 納入本期"}</button> : phase === "voting" ? <button className={`primary-action ${voted ? "selected" : ""}`} onClick={() => toggleVote(p)}>{voted ? (myVotes.some(v => v.product_id === p.id && v.kind === "nomination") ? "▣ 提名票（固定）" : "✓ 已投票（可取消）") : "投一票"}</button> : <span className="rank-label">第 {rankMap.get(p.id)} 名 · {vs.length} 票</span>}</div>{["nomination", "voting"].includes(phase) && <div className="comment-box"><div className="comment-list">{pcs.slice(-3).map(c => <p key={c.id}><strong>{c.author_name}</strong>{c.body}</p>)}{!pcs.length && <small>還沒有人留言，來幫這款拉票吧。</small>}</div><div className="comment-compose"><input maxLength={500} value={commentDrafts[p.id] ?? ""} onChange={e => setCommentDrafts(x => ({ ...x, [p.id]: e.target.value }))} onKeyDown={e => { if (e.key === "Enter") {
        e.preventDefault();
        void addComment(p.id);
    } }} placeholder="留言拉票…"/><button onClick={() => addComment(p.id)}>送出</button></div></div>}</div></article>; })}{!shown.length && <div className="empty-campaign"><h3>{query.trim() || category !== "全部" ? "找不到符合條件的商品" : phase === "nomination" ? "目前還沒有可提名的商品" : "本期目前沒有候選商品"}</h3><p>{query.trim() || category !== "全部" ? "請調整搜尋文字或切換商品分類。" : phase === "nomination" ? "找不到想要的品項時，可以使用上方的「自行新增」。" : "候選商品會在提名完成後顯示。"}</p></div>}</section><EmployeeActivityHistory employee={employee} campaigns={historyCampaigns} products={historyProducts} nominations={historyNominations} votes={historyVotes} purchases={historyPurchases} reviews={purchaseReviews} onSaveReview={savePurchaseReview} onDeleteReview={deletePurchaseReview}/>{toast && <div className="toast">{toast}</div>}{confirmElement}</ShellHeader>;
}
...
function EmployeeActivityHistory({ standalone = false, employee, campaigns, products, nominations, votes, purchases, reviews, reactions = [], onToggleReaction = async () => { }, onSaveReview, onDeleteReview }: {
    ...
    if (!standalone)
        return null;
    if (!campaigns.length)
        return <section id="activity-history" className="employee-history"><div className="history-empty">目前還沒有可回顧的活動；活動產生投票結果或完成商品採購後，就會顯示在這裡。</div></section>;
    if (!selected)
        return null;
    ...
    const discount = Number(selected.purchase_discount_total ?? 0);
    const spent = purchaseTotals(purchasedRows.reduce((sum, row) => sum + Number(row.unit_price) * Number(row.final_quantity ?? row.suggested_quantity), 0), discount).net;
    ...
    <div className="history-context"><p>選擇活動查看最終名次、採購結果與同仁心得。</p><span>顯示最近 {campaigns.length} 期</span></div>
    ...
    <PurchaseReceipt key={selected.id} campaignId={selected.id} expectedDate={selected.purchase_expected_arrival_date}/>
    <div className="history-overview history-overview-with-discount"><div><span>本期預算</span><strong>NT$ {Number(selected.budget).toLocaleString()}</strong></div><div><span>折扣總額</span><strong>NT$ {discount.toLocaleString()}</strong></div><div><span>實際採購（折扣後）</span><strong>NT$ {spent.toLocaleString()}</strong></div><div><span>採購完成度</span><strong>{progress}%</strong></div><div><span>同仁心得</span><strong>{campaignReviews.length} 則</strong></div></div>
...
function EmployeeFeedbackPanel({ employee }: { employee: Employee }) {
    ...
    return <section id="employee-feedback" className="employee-feedback">
      ...
      <div className="feedback-layout"><form onSubmit={submit}>
        ...
        <label>改善建議<textarea name="body" required minLength={5} maxLength={1000} placeholder="哪裡不順、缺少什麼，或你希望下期怎麼調整？"/></label>
...
export function EmployeePurchasePlan({ campaign, products, purchases }: {
    campaign: Campaign;
    products: Product[];
    purchases: PurchaseItem[];
}) { const rows = purchases.filter(row => row.campaign_id === campaign.id && Number(row.final_quantity ?? row.suggested_quantity) > 0).sort((a, b) => a.rank - b.rank || b.vote_count - a.vote_count || a.product_id.localeCompare(b.product_id)); const allPurchased = rows.length > 0 && rows.every(row => row.purchased); const gross = rows.reduce((sum, row) => sum + Number(row.unit_price) * Number(row.final_quantity ?? row.suggested_quantity), 0); const discount = Number(campaign.purchase_discount_total ?? 0); const spent = purchaseTotals(gross, discount).net; const remaining = Number(campaign.budget) - spent; return <section className="employee-purchase-plan"><header><div><p className="section-kicker">LOCKED PURCHASE PLAN</p><h2>本期購買清單</h2><span>{allPurchased ? "✓ 已完成採購" : "採購安排已確認"}</span></div></header><PurchaseReceipt key={campaign.id} campaignId={campaign.id} expectedDate={campaign.purchase_expected_arrival_date}/>{rows.length ? <div className="employee-purchase-list">{rows.map(row => { const product = products.find(item => item.id === row.product_id); const quantity = Number(row.final_quantity ?? row.suggested_quantity); return <article key={row.id}><b>第 {row.rank} 名</b><div><strong>{product ? `${product.brand} ${product.name}` : "商品資料已移除"}</strong><small>{quantity} 份 · 單價 NT$ {Number(row.unit_price).toLocaleString()} · 小計 NT$ {(Number(row.unit_price) * quantity).toLocaleString()}</small></div><span className={row.purchased ? "purchased" : ""}>{row.purchased ? "已採購" : "待採購"}</span></article>; })}</div> : <div className="purchase-plan-empty">清單已鎖定，目前沒有配置採購數量的商品。</div>}{<div className="employee-budget-summary employee-budget-with-discount"><div><span>本期預算</span><strong>NT$ {Number(campaign.budget).toLocaleString()}</strong></div><div><span>清單金額（折扣前）</span><strong>NT$ {purchaseTotals(gross).gross.toLocaleString()}</strong></div><div><span>折扣總額</span><strong>NT$ {discount.toLocaleString()}</strong></div><div><span>{allPurchased ? "實際採購金額（折扣後）" : "清單金額（折扣後）"}</span><strong>NT$ {spent.toLocaleString()}</strong></div><div className={remaining < 0 ? "negative" : ""}><span>{remaining < 0 ? "超出預算" : "預算餘額"}</span><strong>NT$ {Math.abs(remaining).toLocaleString()}</strong></div></div>}</section>; }
...
function AdminApp({ employee }: {
    ...
    async function saveCampaign(e: FormEvent<HTMLFormElement>) { e.preventDefault(); const form = new FormData(e.currentTarget); const id = String(form.get("id") || ""); const wasNew = !id; const previous = campaigns.find(c => c.id === id); const previousLocationIds = campaignLocations.filter(row => row.campaign_id === id).map(row => row.work_location_id); const locationIds = form.getAll("location_ids").map(String); const baseBudget = Number(form.get("budget")); const workLocationId = locationIds[0] || ""; const locationChanged = Boolean(previous && (previousLocationIds.length !== locationIds.length || previousLocationIds.some(locationId => !locationIds.includes(locationId)))); const carryoverEnabled = form.get("carryover_enabled") === "true"; const retainUnusedBudget = form.get("retain_unused_budget") === "true"; const nominationLimit = Number(form.get("nomination_limit")); const voteLimit = Number(form.get("vote_limit")); const startDate = new Date(String(form.get("start_at"))); const nominationDate = new Date(String(form.get("nomination_deadline"))); const votingDate = new Date(String(form.get("voting_deadline"))); const purchaseDate = new Date(String(form.get("purchase_at"))); if (!locationIds.length)
         ...
         return flash("總票數不可少於提名上限"); if ([startDate, nominationDate, votingDate, purchaseDate].some(date => Number.isNaN(date.getTime())))
         return flash("請完整設定四個活動時間"); if (startDate >= nominationDate)
         return flash("提名截止時間必須晚於活動開始時間"); if (nominationDate >= votingDate)
         return flash("投票截止時間必須晚於提名截止時間"); if (votingDate > purchaseDate)
         return flash("安排採購時間不可早於投票截止時間"); const startAt = startDate.toISOString(); const nominationAt = nominationDate.toISOString(); const votingAt = votingDate.toISOString(); const purchaseAt = purchaseDate.toISOString(); let carryoverAmount = 0; let carryoverFrom: string | null = null; let carryoverLabel = ""; let carryoverAvailable = true; if (carryoverEnabled) {
        ...
        carryoverAvailable = detail.retained;
         ...
         return; setBusy(true); const payload = { label: String(form.get("label")), description: String(form.get("description") || "").trim(), budget: totalBudget, base_budget: baseBudget, carryover_enabled: carryoverEnabled, retain_unused_budget: retainUnusedBudget, carryover_amount: carryoverAmount, carryover_from: carryoverFrom, work_location_id: workLocationId, nomination_limit: nominationLimit, vote_limit: voteLimit, start_at: startAt, nomination_deadline: nominationAt, voting_deadline: votingAt, purchase_at: purchaseAt, status: String(form.get("status")), created_by: employee.id }; const result = id ? await supabase.from("campaigns").update(payload).eq("id", id).select("id").single() : await supabase.from("campaigns").insert(payload).select("id").single(); if (result.error) {
         setBusy(false);
         return flash(result.error.message.includes("campaigns_stage_order") ? "活動時間順序不正確，請依序設定開始、提名截止、投票截止與安排採購" : result.error.message);
    ...
    async function saveEditedProduct(form: FormData, image: File | null) { if (!editingProduct)
        ...
        if (form.get("remove_image") === "true") {
            imagePath = null;
        }
        if (image) {
            imagePath = await uploadProductImage(image, employee.id);
        }
        const payload = { brand: String(form.get("brand") || "").trim(), name: String(form.get("name") || "").trim(), category: String(form.get("category") || "").trim(), size: String(form.get("size") || "").trim(), reference_price: form.get("reference_price") ? Number(form.get("reference_price")) : null, image_path: imagePath, image_position_x: Number(form.get("image_position_x") || 50), image_position_y: Number(form.get("image_position_y") || 50), image_fit: String(form.get("image_fit") || "contain"), image_zoom: Number(form.get("image_zoom") || 100), active: String(form.get("active")) === "true" };
    ...
    } const { data, error } = await supabase.rpc("refresh_purchase_plan_prices", { p_campaign_id: purchaseCampaign.id }); setBusy(false); if (error)
        return flash(error.message); const result = data as {
        remaining: number;
        missing_prices: number;
    }; flash(result.missing_prices ? `${purchaseCampaign.label} 已同步商品最新價格，並依前 ${rankLimit} 名產生清單；${result.missing_prices} 項尚未設定價格，因此未配置數量` : `${purchaseCampaign.label} 已同步商品最新價格，並依前 ${rankLimit} 名產生清單，預算餘額 NT$ ${Math.min(Number(purchaseCampaign.budget), Number(result.remaining) + Number(purchaseCampaign.purchase_discount_total ?? 0)).toLocaleString()}`); await load(); }
    ...
    async function updatePurchaseDiscount(campaignId: string, value: number) {
        const { data, error } = await supabase.from("campaigns").update({ purchase_discount_total: value }).eq("id", campaignId).select("id,purchase_discount_total").maybeSingle();
        if (error || !data) {
            flash(error?.message.includes("DISCOUNT_EXCEEDS_TOTAL") ? "折扣不能超過目前清單金額，請重新確認。" : error?.message ?? "折扣未儲存，請確認權限後重試。");
            return false;
        }
        setCampaigns(current => current.map(row => row.id === campaignId ? { ...row, purchase_discount_total: Number(data.purchase_discount_total) } : row));
        return true;
    }
...
{tab === "products" && <>
      ...
      <section className="admin-card"><div className="card-title"><div><p className="section-kicker">ADD PRODUCT</p><h2>新增商品</h2></div></div>
        <form className="product-form product-form-with-image multi-location-form" onSubmit={addProduct}>
          ...
          <ProductImageUploadField compact/>
      ...
      <section className="admin-card"><div className="card-title"><div><p className="section-kicker">PRODUCT CATALOG</p><h2>商品資料庫</h2></div><span className="count-tag">{catalogProducts.length} 項</span></div>
        <div className="table-wrap"><table><thead><tr><th>圖片</th><th>商品</th><th>分類</th><th>適用地點</th><th>參考價</th><th>來源</th><th>狀態</th><th>操作</th></tr></thead><tbody>{catalogProducts.filter(p => p.approval_status !== "pending").map(p => {
          ...
          return <tr key={p.id}><td>{p.image_path ? <img className="product-thumb" src={productImageUrl(p.image_path)} alt="" style={productImageStyle(p)}/> : <span className="thumb-placeholder">{icons[p.category] ?? "✦"}</span>}</td><td><strong>{p.brand} {p.name}</strong><small className="cell-sub">{p.size || "未填規格"}</small></td><td>{p.category}</td><td>{locationNames(ids, workLocations)}</td><td>{p.reference_price == null ? "待確認" : `NT$ ${p.reference_price}`}</td><td>{p.origin === "employee" ? "同仁新增" : "商品庫"}</td><td><span className={p.active ? "active-dot" : "inactive-dot"}>{p.active ? "啟用" : "停用"}</span></td><td><div className="product-row-actions"><button className="table-action" onClick={() => editProduct(p)}>修改</button><label className="table-upload">換圖<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={e => { const file = e.target.files?.[0]; if (file) void replaceProductImage(p, file); }}/></label><button className="table-action" onClick={() => updateProduct(p.id, { active: !p.active }, p.active ? "商品已停用" : "商品已重新啟用")}>{p.active ? "停用" : "啟用"}</button>{canDelete && <button className="table-action danger" disabled={busy} onClick={() => void deleteUnusedProduct(p)}>永久刪除</button>}</div></td></tr>;
        })}</tbody></table></div>
      </section>
    </>}
...
function PendingProductReviewCard({ product, submitterName, locations, selectedLocationIds, busy, onEdit, onSaveLocations, onReview }: {
    ...
    return <article className="pending-review-card">
      {product.image_path ? <img className="review-image" src={productImageUrl(product.image_path)} alt={product.name} style={productImageStyle(product)}/> : <div className="review-icon">{icons[product.category] ?? "✦"}</div>}
...
function ProductEditor({ product, categories, locations, selectedLocationIds, busy, onClose, onSave }: {
    ...
    const [imageZoom, setImageZoom] = useState(Number(product.image_zoom ?? 100));
    const [locationIds, setLocationIds] = useState(selectedLocationIds);
    ...
    return <div className="editor-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) void requestClose(); }}><section className="product-editor" role="dialog" aria-modal="true" aria-labelledby="product-editor-title">
      ...
      <form onSubmit={submit} onChange={() => setDirty(true)}><input type="hidden" name="image_position_x" value={imageX}/><input type="hidden" name="image_position_y" value={imageY}/><input type="hidden" name="image_fit" value={imageFit}/><input type="hidden" name="image_zoom" value={imageZoom}/><div className="editor-layout"><div className="editor-image">{preview ? <ImageFramingControls preview={preview} x={imageX} y={imageY} fit={imageFit} zoom={imageZoom} onChange={next => { if (next.x != null) setImageX(next.x); if (next.y != null) setImageY(next.y); if (next.fit) setImageFit(next.fit); if (next.zoom != null) setImageZoom(next.zoom); setDirty(true); }}/> : <div className="editor-preview"><span>{icons[product.category] ?? "✦"}<small>尚無商品圖</small></span></div>}<label className="image-change">上傳或更換圖片<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={e => selectImage(e.target.files?.[0] ?? null)}/></label>{preview && <button type="button" className="remove-image" onClick={remove}>移除圖片</button>}<small>JPG、PNG、WebP、GIF，最大 5MB</small></div>
...
export function PurchasePanel({ campaign, products, purchases, busy, onGenerate, onUpdate, onSetLocked, onUpdateArrival, onUpdateDiscount }: {
    campaign: Campaign | null;
    products: Product[];
    purchases: PurchaseItem[];
    busy: boolean;
    ...
    onUpdateArrival: (value: string) => Promise<boolean>;
    onUpdateDiscount: (campaignId: string, value: number) => Promise<boolean>;
}) {
    const [receiptPending, setReceiptPending] = useState(true);
    const [arrivalSaving, setArrivalSaving] = useState(false);
    const [discountDraft, setDiscountDraft] = useState(String(campaign?.purchase_discount_total ?? 0));
    const [discountState, setDiscountState] = useState<"idle" | "saving" | "saved" | "error">("idle");
    const discountSaving = useRef(false);
    useEffect(() => { setDiscountDraft(String(campaign?.purchase_discount_total ?? 0)); }, [campaign?.id, campaign?.purchase_discount_total]);
    const gross = purchaseTotals(purchases.filter(row => row.campaign_id === campaign?.id).reduce((sum, row) => sum + Number(row.unit_price) * Number(row.final_quantity ?? row.suggested_quantity), 0)).gross;
    const discountValid = validPurchaseDiscount(discountDraft, gross);
    const discountDirty = !discountValid || Number(discountDraft) !== Number(campaign?.purchase_discount_total ?? 0);
    async function saveDiscount() {
        if (!campaign || discountSaving.current) return;
        if (!discountValid) { setDiscountState("error"); return; }
        if (!discountDirty && discountState !== "error") return;
        discountSaving.current = true; setDiscountState("saving");
        try { setDiscountState(await onUpdateDiscount(campaign.id, Number(discountDraft)) ? "saved" : "error"); }
        catch { setDiscountState("error"); }
        finally { discountSaving.current = false; }
    }
    ...
    return <section className="admin-card"><div className="admin-empty">請先建立本期活動。</div></section>; const rows = purchases.filter(p => p.campaign_id === campaign.id); const total = rows.reduce((sum, row) => sum + Number(row.unit_price) * Number(row.final_quantity ?? row.suggested_quantity), 0); const discount = discountValid ? Number(discountDraft) : Number(campaign.purchase_discount_total ?? 0); const netTotal = purchaseTotals(total, discount).net; const remaining = Math.round((Number(campaign.budget) - netTotal) * 100) / 100; const locked = Boolean(campaign.purchase_plan_locked_at); const buyingRows = rows.filter(row => Number(row.final_quantity ?? row.suggested_quantity) > 0); const completed = locked && buyingRows.length > 0 && buyingRows.every(row => row.purchased); const hasPending = arrivalSaving || pendingRows.size > 0 || discountDirty || discountState === "saving" || discountState === "error"; const rankLimit = Number(rankLimitDraft); const validRankLimit = Number.isInteger(rankLimit) && rankLimit >= 1 && rankLimit <= 100; return <><div className="purchase-summary purchase-summary-with-discount"><div><span>本期可用預算 · {campaign.label}</span><strong>NT$ {Number(campaign.budget).toLocaleString()}</strong><small>基本 NT$ {Number(campaign.base_budget ?? campaign.budget).toLocaleString()}{campaign.carryover_enabled ? ` ＋ 使用上期保留 NT$ ${Number(campaign.carryover_amount).toLocaleString()}` : " · 未使用上期保留"}{campaign.retain_unused_budget ? " · 本期餘額將保留" : " · 本期餘額不保留"}</small></div><div><span>目前清單金額</span><strong>NT$ {purchaseTotals(total).gross.toLocaleString()}</strong><small>折扣前合計</small></div><div className="purchase-discount-card"><label htmlFor={`discount-${campaign.id}`}>折扣總額</label><div className="discount-input-wrap"><span>NT$</span><input id={`discount-${campaign.id}`} aria-describedby={`discount-note-${campaign.id}`} aria-invalid={!discountValid || discountState === "error"} type="number" min="0" max={gross} step="0.01" inputMode="decimal" value={discountDraft} disabled={busy || discountState === "saving"} onChange={e => { setDiscountDraft(e.target.value); setDiscountState("idle"); }} onBlur={() => void saveDiscount()} onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); e.currentTarget.blur(); } }}/></div><small id={`discount-note-${campaign.id}`} role="status" className={discountState === "error" || !discountValid ? "discount-error" : ""}>{!discountValid ? "請輸入 0 至清單金額，最多兩位小數" : discountState === "saving" ? "儲存中…" : discountState === "error" ? "儲存失敗，請重新點入欄位後離開以重試" : discountState === "saved" && !discountDirty ? "已儲存" : "離開欄位或按 Enter 儲存"}</small></div><div className={remaining < 0 ? "over-budget" : ""}><span>{remaining < 0 ? "超出預算" : "預算餘額"}</span><strong>NT$ {Math.abs(remaining).toLocaleString()}</strong><small>折扣後金額 NT$ {netTotal.toLocaleString()}{discountDirty && discountValid ? "（預覽，尚未儲存）" : ""}</small></div></div><section className="admin-card"><div className="card-title purchase-title"><div><p className="section-kicker">PURCHASE PLAN</p><h2>{campaign.label} 採購清單</h2><span className={`purchase-plan-state ${completed ? "completed" : locked ? "locked" : "draft"}`}>{completed ? "✓ 本期採購完成" : locked ? "已鎖定 · 待採購" : "草稿 · 票數異動時自動更新"}</span></div><div className="purchase-plan-actions"><label className="purchase-rank-limit"><span>建議採購名次</span><span className="rank-limit-input">前 <input aria-label="採購名次上限" type="number" min="1" max="100" step="1" value={rankLimitDraft} disabled={locked || busy} onChange={e => setRankLimitDraft(e.target.value)}/> 名</span></label>{locked && <label className="arrival-editor"><span>預計到貨日期（選填）</span><input type="date" value={campaign.purchase_expected_arrival_date ?? ""} disabled={arrivalSaving || busy} onChange={async e => { setArrivalSaving(true); try { await onUpdateArrival(e.target.value); } finally { setArrivalSaving(false); } }}/></label>}<button className="recalculate-button" disabled={locked || hasPending || busy || !validRankLimit} onClick={() => onGenerate(rankLimit)}>{rows.length ? "重新計算建議" : "產生採購建議"}</button>{rows.length && <button className={locked ? "unlock-button" : "lock-button"} disabled={hasPending || busy} onClick={() => onSetLocked(!locked)}>{locked ? "解鎖修改" : "儲存並鎖定清單"}</button>}</div></div><PurchasePdfDownload key={`pdf-${campaign.id}`} campaignId={campaign.id} disabled={!locked || !buyingRows.length || busy || hasPending || receiptPending} disabledReason={!locked ? "請先儲存並鎖定採購清單。" : !buyingRows.length ? "目前沒有採購品項。" : "請先完成欄位儲存，並等待到貨紀錄讀取完成。"}/>{locked && <PurchaseReceipt key={campaign.id} campaignId={campaign.id} expectedDate={campaign.purchase_expected_arrival_date} editable onPendingChange={setReceiptPending}/>}{hasPending && <div className="purchase-pending-note">尚有欄位未儲存；請按 Enter 或點到欄位外，完成儲存後即可鎖定。</div>}<div className="rule-note purchase-rule"><strong>{locked ? `清單已固定 · 取前 ${campaign.purchase_plan_rank_limit ?? rankLimit} 名` : "分配規則"}</strong><span>{locked ? "單價、數量及排名已鎖定。請依實際採購進度逐項標記「已採購」；如需變更內容，請先解鎖。" : `未鎖定期間，票數變更會自動重算前 ${validRankLimit ? rankLimit : "—"} 名及採購數量；同票並列名次會一併納入。自動更新時保留人工調整單價；按「重新計算建議」會以商品清單最新參考價格覆蓋採購單價，並重算數量與金額。`}</span></div>{rows.length ? <div className="table-wrap"><table><thead><tr><th>排名</th><th>商品</th><th>票數</th><th>採購單價</th><th>採購數量</th><th>小計</th><th>採購狀態</th></tr></thead><tbody>{rows.map(row => <PurchaseRow key={row.id} row={row} product={products.find(p => p.id === row.product_id)} locked={locked} onUpdate={onUpdate} onPendingChange={reportPending}/>)}</tbody></table></div> : <div className="admin-empty">投票結束後，確認要採用的名次範圍，再按「產生採購建議」。請先在商品資料庫確認入選商品的參考價格。</div>}</section></>; }
