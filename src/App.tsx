...
function ImageFramingControls({ preview, x, y, fit, onChange }: { preview: string; x: number; y: number; fit: "cover" | "contain"; onChange: (next: { x?: number; y?: number; fit?: "cover" | "contain" }) => void; }) {
    ...
    return <div className="image-framing-panel"><div className="product-visual image-framing-preview has-image"><img src={preview} alt="商品圖片顯示預覽" style={{ objectPosition: `${x}% ${y}%`, objectFit: fit }}/><span>前台商品卡片實際畫面</span></div><div className="image-framing-controls"><div className="fit-choice"><button type="button" className={fit === "cover" ? "active" : ""} onClick={() => onChange({ fit: "cover" })}>填滿視窗</button><button type="button" className={fit === "contain" ? "active" : ""} onClick={() => onChange({ fit: "contain" })}>完整顯示</button></div><label><span>左右焦點</span><input type="range" min="0" max="100" value={x} onChange={event => onChange({ x: Number(event.target.value) })}/><output>{x}%</output></label><label><span>上下焦點</span><input type="range" min="0" max="100" value={y} onChange={event => onChange({ y: Number(event.target.value) })}/><output>{y}%</output></label><small>此處直接使用前台商品圖片元件。「填滿視窗」可調整焦點；「完整顯示」會保留整張圖片。</small></div></div>;
}
...
function ProductEditor({ product, categories, locations, selectedLocationIds, busy, onClose, onSave }: {
    ...
    return <div className="editor-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) void requestClose(); }}><section className="product-editor" role="dialog" aria-modal="true" aria-labelledby="product-editor-title">
      ...
      <form onSubmit={submit} onChange={() => setDirty(true)}><input type="hidden" name="image_position_x" value={imageX}/><input type="hidden" name="image_position_y" value={imageY}/><input type="hidden" name="image_fit" value={imageFit}/><div className="editor-layout"><div className="editor-image">{preview ? <ImageFramingControls preview={preview} x={imageX} y={imageY} fit={imageFit} onChange={next => { if (next.x != null) setImageX(next.x); if (next.y != null) setImageY(next.y); if (next.fit) setImageFit(next.fit); setDirty(true); }}/> : <div className="editor-preview"><span>{icons[product.category] ?? "✦"}<small>尚無商品圖</small></span></div>}<label className="image-change">上傳或更換圖片<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={e => selectImage(e.target.files?.[0] ?? null)}/></label>{preview && <button type="button" className="remove-image" onClick={remove}>移除圖片</button>}<small>JPG、PNG、WebP、GIF，最大 5MB</small></div>
