...
function AdminApp({ employee }: {
    ...
    }; flash(result.missing_prices ? `${purchaseCampaign.label} 已依前 ${rankLimit} 名產生清單；${result.missing_prices} 項尚未設定價格，因此未配置數量` : `${purchaseCampaign.label} 已依前 ${rankLimit} 名產生清單，預算餘額 NT$ ${Math.min(Number(purchaseCampaign.budget), Number(result.remaining) + Number(purchaseCampaign.purchase_discount_total ?? 0)).toLocaleString()}`); await load(); }
