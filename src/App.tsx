...
function productImageStyle(product: Pick<Product, "image_position_x" | "image_position_y" | "image_fit" | "image_zoom">) { const x = Number(product.image_position_x ?? 50); const y = Number(product.image_position_y ?? 50); return { objectPosition: `${x}% ${y}%`, objectFit: product.image_fit === "cover" ? "cover" as const : "contain" as const, transform: `scale(${Number(product.image_zoom ?? 100) / 100})`, transformOrigin: `${x}% ${y}%` }; }
function ImageFramingControls({ preview, x, y, fit, zoom, onChange }: { preview: string; x: number; y: number; fit: "cover" | "contain"; zoom: number; onChange: (next: { x?: number; y?: number; fit?: "cover" | "contain"; zoom?: number }) => void; }) {
    ...
    return <div className="image-framing-panel"><div className="product-visual image-framing-preview has-image"><img src={preview} alt="商品圖片顯示預覽" style={{ objectPosition: `${x}% ${y}%`, objectFit: fit, transform: `scale(${zoom / 100})`, transformOrigin: `${x}% ${y}%` }}/><span>前台商品卡片實際畫面</span></div><div className="image-framing-controls"><div className="fit-choice"><button type="button" className={fit === "cover" ? "active" : ""} onClick={() => onChange({ fit: "cover", zoom: 100 })}>填滿視窗</button><button type="button" className={fit === "contain" ? "active" : ""} onClick={() => onChange({ fit: "contain", zoom: 100 })}>完整顯示</button></div><label><span>圖片縮放</span><input type="range" min="100" max="250" step="5" value={zoom} onChange={event => onChange({ zoom: Number(event.target.value) })}/><output>{zoom}%</output></label><label><span>左右焦點</span><input type="range" min="0" max="100" value={x} onChange={event => onChange({ x: Number(event.target.value) })}/><output>{x}%</output></label><label><span>上下焦點</span><input type="range" min="0" max="100" value={y} onChange={event => onChange({ y: Number(event.target.value) })}/><output>{y}%</output></label><small>先用「完整顯示」看見整張圖，再依需要放大並調整焦點；此處就是前台實際畫面。</small></div></div>;
}
